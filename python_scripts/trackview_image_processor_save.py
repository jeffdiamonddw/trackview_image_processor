...
def get_epsg_from_latlon(lat, lon):
    """
    Takes latitude and longitude and returns the WGS84 UTM EPSG code.
    ...
    Returns:
        str: The EPSG code (e.g., 'epsg:32611')
    """
    # utm.from_latlon returns: (easting, northing, zone_number, zone_letter)
    _, _, zone_number, zone_letter = utm.from_latlon(lat, lon)
    
    # In the UTM system, letters N and above are in the Northern Hemisphere
    # This matches the logic in your trackview_image_processor.py
    if zone_letter >= 'N':
        # Northern Hemisphere WGS84 UTM base is 32600
        epsg_code = 32600 + zone_number
    else:
        # Southern Hemisphere WGS84 UTM base is 32700
...
def process_image(image_path, temp_dir = '/tmp', transparency_buffer = .05):
    ...
    elif re.search("(superwake|trinity|quantum)", request_dict['info']['drone_model'].lower()) is not None:
        
        result = extract_phaseone_orientation(image_path)
        image_data.update(result)   


    epsg_code = get_epsg_from_latlon(image_data['lat'], image_data['lon'])
    image_data['crs'] = epsg_code
   
 
    df_point = gpd.GeoDataFrame().set_geometry([Point(image_data['lon'], image_data['lat'])]).set_crs('epsg:4326').to_crs(epsg_code)
    image_data['x'] = df_point.geometry.iloc[0].x
    ...
    print({key: key in image_data for key in get_arguments(get_world_boundary)})
